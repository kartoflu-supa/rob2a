...
#pragma config(Sensor, dgtl4,  front_Bumper,   sensorTouch)
#pragma config(Sensor, dgtl5,  claw_Limit,     sensorTouch)
...
task safety(){
	while (1==1){
		if (vexRT[Btn8U] == 1 || SensorValue[front_Bumper] == 1){
			StopAllTasks();
		}
	}
}
...
void turn20(bool l_r) {
	int turn_num = BASE_DEG*20;
	resetEncoder();
	if (l_r) {
...
task main()
{
	...
	while (1==1) {
			...
			motor[leftMotor]=FULL_FORWARD;
			motor[rightMotor]=FULL_FORWARD;
	}
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

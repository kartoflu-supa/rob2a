...
task watch(){
	while(true){
		while(SensorValue[midLine] > 500){
			motor[leftMotor] = 63;
			motor[rightMotor] = 63;
		if (SensorValue[rightLine] > 500){
			r_s = true;
		if (SensorValue[leftLine] < 500){
			l_s = false;
		}
		}
		if (SensorValue[leftLine] > 500){
		...
		if (SensorValue[rightLine] > 500 || r_s == true){
			motor[leftMotor] = 63;
			motor[rightMotor] = 82;
		}
		if (SensorValue[leftLine] > 500 || l_s == true){
			motor[leftMotor] = 82;
			motor[rightMotor] = 63;
		}
		}
